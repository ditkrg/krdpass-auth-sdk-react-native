...
attacking never crosses a network. The authorization request is handed app to app through
the platform's own explicit-launch mechanism to a KRDPASS app whose signing certificate the
native SDK does pin. The HTTPS calls that remain are validated against the platform trust
store.
