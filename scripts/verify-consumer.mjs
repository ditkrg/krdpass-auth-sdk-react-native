...
const BARE_REACT_NATIVE_VERSION =
  process.env.BARE_REACT_NATIVE_VERSION || "0.86.0";
const REACT_NATIVE_CLI_VERSION =
  process.env.REACT_NATIVE_CLI_VERSION || "20.2.0";
const BARE_ARCHITECTURE = process.env.BARE_ARCHITECTURE;
...
function createBareConsumer(consumerRoot, tarball) {
  command(
    npx,
    [
      "--yes",
...
function verifyExpoPlugin(consumerRoot) {
  ...
  const podfile = readFileSync(join(consumerRoot, "ios", "Podfile"), "utf8");
  if (!podfile.includes("pod 'KrdpassAuth', :git => 'https://github.com/ditkrg/krdpass-auth-sdk-ios.git', :tag => 'v1.1.0'")) {
...
function buildAndroid(consumerRoot) {
  ...
  const architectureArgument =
    BARE_ARCHITECTURE === "legacy"
      ? "-PnewArchEnabled=false"
      : BARE_ARCHITECTURE === "new"
  ...
  command(
    ...
    [
      "--no-daemon",
      ...(architectureArgument ? [architectureArgument] : []),
      ":app:assembleDebug",
    ],
...
const { kind, platform } = parseArguments();
...
try {
  ...
  if (kind === "bare") createBareConsumer(consumerRoot, tarball);
  else createExpoConsumer(consumerRoot, tarball);
  if (platform === "android") buildAndroid(consumerRoot);
  else buildIos(consumerRoot);
  process.stdout.write(`\n${kind} ${platform} consumer verification passed.\n`);
} finally {
