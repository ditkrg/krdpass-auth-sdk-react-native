...
describe("package metadata", () => {
  ...
  it("declares an Android package name matching the native module's source tree", () => {
    // A mismatch compiles on both sides and fails at runtime with an unresolved TurboModule.
  ...
  it("exposes only the package root, so build/ and src/ are not deep-importable", () => {
    // Without an exports map every internal module becomes a semver commitment.
...
describe("version is bumped everywhere or nowhere", () => {
  // The version is hardcoded in every file below, and release.yml only checks the git tag
  // against package.json; a partial bump would ship a wrapper pinned to a stale native core.
